...
- <i class="fas fa-chalkboard-teacher"></i> **Slides** are "[Xaringan](https://github.com/yihui/xaringan)" presentations in html that can be opened in any browser (You can find a downloadable PDF in each respective **content** page)
...
sched <- read_csv('data/schedule.csv', show_col_types = FALSE) %>%
    ...
    mutate(var_practice = ifelse(!is.na(practice),
                                glue('<a href="{practice}.qmd"><i class="fas fa-dumbbell"></i></a>'),
                                glue('<font color="#e9ecef"><i class="fas fa-dumbbell"></i></font>'))) %>%
...
sched %>%
    select(unit, ` ` = col_date, Title = col_title, Content = col_content,
           Practice = col_practice, Assignment = col_assignment) %>%
...
nested_sched <- sched %>%
    select(unit, ` ` = col_date, Title = col_title, Content = col_content,
           Slides = var_slides, Practice = col_practice, Assignment = col_assignment) %>%
  group_by(unit) %>%
  nest()
```
